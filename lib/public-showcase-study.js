...
export function getPublicStudyActionState() {
  return {
    canSubmit: false,
    canAutosave: false,
    submitMessage: "可在线继续记录研习内容并整理完善，填写结果会保存在当前浏览器中，完成后可导出留存。",
    autosaveMessage: "如需保留研习内容，可继续补充并导出记录留存。",
  };
}

export function getPublicStudyHeadNote({ hasPdf } = {}) {
  return hasPdf
    ? "围绕同一份裁判文书完成站内阅读、梳理与分析，研习内容可继续保存在当前浏览器中。"
    : "先结合案例导读把握事实与争点，再进入结构化研习并完成三步写作。";
}

export function getPublicStudyReaderNote({ hasPdf }) {
  if (hasPdf) {
    return "优先在站内阅读原文版式与关键段落；如当前环境受限，也可使用备用打开方式继续查看。";
  }
...
export function normalizePublicStudyFeedback(item) {
  ...
  return {
    ...item,
    title: item.title || "研习说明",
    tone: item.tone === "success" ? "success" : "soft",
    message: "可继续在线整理研习内容，完成后可导出记录留存。",
  };
}
