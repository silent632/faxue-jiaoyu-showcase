...
const COURSE_PACKAGE_ROOT_CANDIDATES = [
  process.env.COURSE_PACKAGE_ROOT,
  path.join(homedir(), "Desktop", "双师项目", "课程资料包"),
  path.join(homedir(), "Documents", "双师项目", "课程资料包"),
  path.resolve(process.cwd(), "课程资料包"),
  path.resolve(process.cwd(), "../课程资料包"),
].filter(Boolean);
...
const COURSE_GUIDE_FALLBACKS = {
  ...
  第四期: {
    courseTheme: "从“沉睡的权利”到正当救济的司法考量",
    teachingTitle: "从“沉睡的权利”到正当救济的司法考量",
    coursePosition: "围绕权利救济与诉讼时效问题，训练学生在真实裁判中理解权利行使、救济边界与裁判说理。",
    audience: "法学本科二、三年级学生",
    caseDirections: [],
    preStudy: [],
    goals: [],
  ...
  第五期: {
    courseTheme: "非法证据排除规则",
    teachingTitle: "为什么非法证据必须排除",
    coursePosition: "围绕非法证据排除规则，讨论程序正义如何限制国家取证权力，并训练证据能力审查判断。",
    audience: "法学本科二、三年级学生",
    caseDirections: [],
    preStudy: [],
    goals: [],
  ...
  第六期: {
    courseTheme: "个人信息保护与技术治理",
    teachingTitle: "人脸识别中的同意边界与法理回应",
    coursePosition: "围绕人脸识别场景，拆解明示同意、默示同意与拒绝权的同意边界，聚焦人格权保护的判断标准。",
    audience: "法学本科二、三年级学生",
    caseDirections: [],
    preStudy: [],
    goals: [],
  ...
  第七期: {
    courseTheme: "平台劳动关系认定中的法理冲突",
    teachingTitle: "接单自由，还是算法控制",
    coursePosition: "围绕平台用工与算法控制问题，讨论劳动关系认定、管理从属性与平台治理责任。",
    audience: "法学本科二、三年级学生",
    caseDirections: [],
    preStudy: [],
    goals: [],
  ...
  第八期: {
    courseTheme: "生成式 AI 的作品认定与责任边界",
    teachingTitle: "当机器开始“创作”",
    coursePosition: "围绕生成式 AI 文图内容，讨论作品认定标准与平台、使用者、模型提供方之间的责任边界。",
    audience: "法学本科二、三年级学生",
    caseDirections: [],
    preStudy: [],
    goals: [],
...
const PERIOD_OUTLINE_FALLBACKS = {
  ...
  第五期: ["规则导入", "取证权力边界", "排除条件", "举证责任", "裁判说理", "程序正义收束"],
  第六期: ["场景导入", "人脸识别应用", "同意边界", "拒绝权保护", "人格权判断", "技术治理回应"],
  第七期: ["平台用工场景", "算法控制", "从属性判断", "劳动关系认定", "平台责任", "治理边界"],
  第八期: ["生成式 AI 场景", "作品认定", "独创性判断", "侵权风险", "责任分配", "平台治理"],
};
...
function normalizeMentorValue(period, role, value) {
  ...
  const normalizedValue = normalizeMentorIdentity(trimmed);
  const fallbackName = fallback.replace(/（.+）/u, "").trim();
  const normalizedFallbackName = normalizeMentorIdentity(fallbackName);

  if (normalizedValue && normalizedFallbackName && normalizedValue === normalizedFallbackName) {
    return fallback;
  }
...
function buildGuideForPeriod(period, periodDir) {
  ...
  const guide = readGuideSections(guideFile) || COURSE_GUIDE_FALLBACKS[period] || {
    courseTheme: "",
    teachingTitle: "",
    coursePosition: "",
    audience: "法学本科二、三年级学生",
  ...
  return {
    ...guide,
    theoryMentor: normalizeMentorValue(period, "theoryMentor", guide.theoryMentor),
    practiceMentor: normalizeMentorValue(period, "practiceMentor", guide.practiceMentor),
  };
}
...
function buildPeriodLookup() {
  return getShowcaseVideoPeriods().map((video) => {
    ...
    const courseSystemPages = buildCourseSystemPages({
      ...basePeriod,
      archiveCard,
      detailContent,
      courseContentProfile,
    });
    const courseMaterialPages = buildCourseMaterialPages({
      ...basePeriod,
      archiveCard,
      detailContent,
      courseContentProfile,
