...
function buildPeriodHome(period) {
  ...
  const summaries = profile.sectionSummaries || {};

  return {
    summary: profile.periodSummary.lead,
    highlights: uniqueTexts(profile.periodSummary.keySignals, 4),
    materialClues: uniqueTexts(profile.caseStudy.mainCases, 3),
    cards: COURSE_PERIOD_SECTIONS.map((section) => {
      const summary = summaries[section.key] || {};
      return {
        key: section.key,
        label: section.label,
        summary: summary.summary || section.label,
        detail: summary.detail || profile.periodSummary.lead,
