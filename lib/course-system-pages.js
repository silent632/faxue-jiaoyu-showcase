...
function sanitizeCourseSystemCopy(value) {
  return String(value || "")
    .replace(/章节配音稿与内容导图/gu, "课程主线与结构图")
...
function renderMaterialEntries(entries) {
  return uniqueTexts(
    entries.map((entry) => {
      if (!entry) return "";
      return `${sanitizeCourseSystemCopy(entry.title)}：${sanitizeCourseSystemCopy(entry.content)}`;
    })
  );
}
