...
function buildReportSections(period) {
  ...
  const paths = uniqueTexts(profile.caseStudy.analysisPath, 3);
  ...
  const sampleCards = questions.map((question, index) => ({
    title: question,
    paragraphs: uniqueTexts([findings[index] || findings[0], paths[index] || paths[0]], 2),
  }));
