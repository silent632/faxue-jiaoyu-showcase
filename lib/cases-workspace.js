...
function listParamValues(value) {
...
export function resolveCasesStateFromSearchParams(searchParamsLike) {
  const getValue = (key) => {
    if (searchParamsLike instanceof URLSearchParams) return searchParamsLike.get(key) || "";
    return firstParamValue(searchParamsLike?.[key]);
  };
  const getList = (key) => {
    if (searchParamsLike instanceof URLSearchParams) {
      return listParamValues(searchParamsLike.get(key) || "");
    }
    return listParamValues(searchParamsLike?.[key]);
  };

  const filters = normalizeInitialCaseFilters({
    keyword: getValue("q"),
    causeL1: getValue("causeL1"),
    causeL2: getValue("causeL2"),
    causeL3: getValue("causeL3"),
    courtLevel: getList("courtLevel"),
    year: getValue("year"),
    docType: getList("docType"),
    procedure: getList("procedure"),
    province: getList("province"),
    result: getList("result"),
    lawName: getList("lawName"),
    lawArticle: getList("lawArticle"),
  });

  return {
    filters,
    keywordInput: filters.keyword,
    sortMode: resolveSortMode(getValue("sort")),
    currentPage: resolvePageNumber(getValue("page") || "1"),
  };
}
...
export function buildStarterActions({ filtered, rows, filters }) {
  ...
  if (!filters.year) {
    pushCandidates("year", scopeRows.map((item) => item.judgmentDate.slice(0, 4)).filter((value) => /^\d{4}$/.test(value)));
  }
