...
export function getStudyWorkspaceMetrics({ caseItem, draft }) {
  const normalizedDraft = normalizeStudyDraft(draft);
  ...
  return {
    ...
    completionText:
      doneCount === STUDY_STEP_DEFINITIONS.length
        ? "三步都已完成，可以导出公开展示版研习记录。"
        : "建议按“事实梳理 → 争议焦点 → 法理分析”的顺序推进。",
...
export function getPublicStudyActionState() {
  return {
    ...
    autosaveMessage: "草稿会保存在当前浏览器中，方便继续演示。",
  };
}
...
export async function submitStudyWorkspace() {
  return {
    ...
    tone: "warning",
    title: "公开展示模式",
    message: "当前为公开展示模式：页面仅用于展示真实研习工作台结构，不开放真实提交；你填写的内容仍会保留在当前浏览器中。",
  };
}
...
export function buildStudyReportFileName(caseItem) {
  ...
  return `${toSafeFileName(caseLabel)}-${toSafeFileName(caseTitle)}-公开研习记录.txt`;
}
