...
function ensureArray(value) {
  ...
  return value.filter((item) => typeof item === "string" ? item.trim().length > 0 : Boolean(item));
}
...
function buildArchiveKeyPoints(raw, fallback) {
  const points = [];
  appendUnique(points, ensureArray(raw.outline));
  appendUnique(points, ensureArray(raw.guide?.highlights));
  appendUnique(points, fallback?.keyPoints ?? []);
  if (points.length < 2) {
    appendUnique(points, DEFAULT_ARCHIVE_CARD.keyPoints);
  }
  return points.slice(0, Math.max(2, points.length));
}
...
function buildArchiveCard(period, raw, isLate) {
  ...
  const lead = trimText(raw.summary) || trimText(raw.guide?.coursePosition) || fallback.lead;
  const keyPoints = buildArchiveKeyPoints(raw, fallback);
...
function buildDetailIntro(raw, isLate) {
  ...
  return lines.slice(0, Math.max(2, lines.length));
}
...
function buildOutlineSections(raw) {
  ...
  if (sections.length < 3) {
    ...
    sections.push(...DEFAULT_DETAIL_CONTENT.sections.slice(0, needed));
  }
  return sections;
}
...
function buildMaterialHighlights(raw, isLate) {
  ...
  if (isLate) {
    ...
    return [...highlights, ...DEFAULT_DETAIL_CONTENT.materialHighlights].slice(0, 2);
  }
  const courseGroup = (raw.materialGroups || []).find((group) => group.title === "课程资料");
  if (courseGroup?.items?.length) {
    const items = courseGroup.items.slice(0, 2);
    return items.map((item) => ({
      title: item.displayName,
      summary: describeMaterialHighlight(item, theme),
    }));
  }
  return DEFAULT_DETAIL_CONTENT.materialHighlights;
}
...
function buildTeachingDesign(raw, isLate) {
  ...
  const bullets = []; 
  appendUnique(bullets, ensureArray(raw.guide?.highlights));
  if (raw.guide?.theoryMentor) {
    bullets.push(`理论导师 ${raw.guide.theoryMentor} 负责方法与争点框架。`,);
  }
  if (raw.guide?.practiceMentor) {
    bullets.push(`实务导师 ${raw.guide.practiceMentor} 结合裁判经验补充路径。`,);
  }
  ...
  return { summary, bullets: bullets.slice(0, Math.max(2, bullets.length)) };
}
