...
const DEFAULT_SOURCE_ROOTS = [path.resolve(PROJECT_ROOT, "..", "faxue-jiaoyu"), path.resolve(PROJECT_ROOT, "..", "faxue-jiaoyu-education")];
...
export function resolvePdfSyncSourceDir({ sourceDir, sourceRoot } = {}) {
  const explicitSourceDir = String(sourceDir || process.env.SHOWCASE_PDF_SOURCE_DIR || "").trim();
  if (explicitSourceDir) return explicitSourceDir;

  const candidates = [sourceRoot, process.env.SHOWCASE_SOURCE_ROOT, ...DEFAULT_SOURCE_ROOTS]
    .map((item) => String(item || "").trim())
    .filter(Boolean)
    .map((root) => path.join(root, "public", "pdfs"));
  ...
  for (const candidate of candidates) {
    ...
    try {
      ...
      if (!isMissingFileError(error)) {
        throw new Error(
          `Failed to copy PDF "${candidate.fileName}" for case "${candidate.id}" from ${sourcePath} to ${targetPath}: ${String(error?.message || error)}`,
          { cause: error }
        );
      }
...
async function main() {
  const sourceDir = resolvePdfSyncSourceDir();
