...
const PHASE_GUIDE = [
  {
    ...
    title: "前四期：课堂实施与机制验证",
    description: "以案例进入、课堂互动和双师协作过程为主线，集中呈现前期课程的实施形态与课堂推进节奏。",
  },
  {
    slug: "phase-resource",
    title: "后四期：示范性课程视频与资源化成果",
    description: "在前期课程经验基础上转入视频化表达、展示共享与成果沉淀，形成更稳定的课程展示资源。",
  },
];
...
const COURSE_VIDEO_PERIODS = [
  buildVideoPeriod({
    ...
    purpose: "本期保留分段课堂记录，进入本期说明页后可按顺序查看各段内容。",
    segments: [
      ...
      buildSegment("第一期下", "非法证据排除规则", "https://llm2x7.58u.cn/a/Rn83yvz/", "将规则理解与案例讨论进一步结合。"),
    ],
  }),
  buildVideoPeriod({
    ...
    purpose: "本期同样保留分段入口，进入本期说明页后可依次查看上、下两段内容。",
    segments: [
  ...
  buildVideoPeriod({
    ...
    summary: "围绕权利救济与规范适用，集中呈现前四期课堂实施成果的收束与方法训练。",
  ...
  buildVideoPeriod({
    ...
    summary: "在前期课程经验基础上转入程序法专题，形成更适合展示共享的课程视频成果。",
...
export function buildShowcaseHomeVideoBlock() {
  return {
    title: "八期课程视频成果",
    description: "课程视频页按八期顺序集中展示课程成果，并用阶段说明区分前四期课堂实施成果与后四期示范性课程视频、资源化成果。",
    phaseGuide: getShowcaseVideoPhaseGuide(),
    periods: getShowcaseVideoPeriods(),
    segmentNotice: {
      title: "第一期、第二期分段内容已下沉到本期说明页",
      description: "首页不直接铺开全部分段入口，进入对应期次后再查看分段内容，更能保持八期主线的连续感。",
    },
  };
}
